...
import * as platterService from "./platter.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await platterService.createPlatter({
    ...req.body,
  ...
  successResponse(res, 200, "Platter created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await platterService.getAllList(page, limit, search);
  await Promise.all(
  ...
  successResponse(res, 200, "Platter records fetch successfully", records);
});

export const getOne = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const record = await platterService.getPlatterById(id);
  ...
  successResponse(res, 200, "Get edit platter record", record);
});

export const update = asyncHandler(async (req: Request, res: Response) => {
  const user = req.user as { id?: string };
  const id = req.params.id as string;
  const oldRecord = await platterService.getPlatterById(id);
  ...
  const updatedRecord = await platterService.updatePlatter(id, updatePayload);
  ...
  successResponse(res, 200, "Platter updated successfully", updatedRecord);
});

export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await platterService.getPlatterById(id);

  if (!item) {
    throw new ApiError(404, "Platter record not found");
  }
  ...
  await platterService.deletePlatterById(id);
  successResponse(res, 200, "Platter record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await platterService.updatePlatterSeq(id, payload);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (platterService as any).getById
      ? await (platterService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (platterService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
