...
import type { IAmenitiesUpdateDTO } from "./amenities.interface.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await amenitiesService.createAmenities({
    ...req.body,
  ...
  successResponse(res, 200, "Amenities created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await amenitiesService.getAllList(page, limit, search);
  ...
  successResponse(res, 200, "Amenities records fetched successfully", records);
});

export const getOne = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const record = await amenitiesService.getAmenitiesById(id);
  ...
  successResponse(res, 200, "Get amenities record", record);
});
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await amenitiesService.getAmenitiesById(id);
  ...
  const updatePayload: IAmenitiesUpdateDTO = {
  ...
  const updatedRecord = await amenitiesService.updateAmenities(
    id,
    updatePayload,
  );
  ...
  successResponse(res, 200, "Amenities updated successfully", updatedRecord);
});

export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await amenitiesService.getAmenitiesById(id);

  if (!item) {
    throw new ApiError(404, "Amenities record not found");
  }
  ...
  await amenitiesService.deleteAmenities(id);
  successResponse(res, 200, "Amenities record deleted successfully");
});
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (amenitiesService as any).getById
      ? await (amenitiesService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (amenitiesService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
