...
import * as csrContentDetailService from "./csrContentDetails.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await csrContentDetailService.createCsrContentDetail({
    ...req.body,
  ...
  successResponse(res, 200, "CSR Content created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await csrContentDetailService.getAllList(
    page,
    limit,
    search,
  );
  ...
  successResponse(
    res,
    200,
    "CSR Content records fetched successfully",
    records,
  );
});
...
export const getOne = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await csrContentDetailService.getCsrContentDetailById(id);
  ...
  successResponse(res, 200, "Get edit CSR Content record", record);
});

export const update = asyncHandler(async (req: Request, res: Response) => {
  const user = req.user as { id?: string };
  const id = req.params.id as string;
  const oldRecord = await csrContentDetailService.getCsrContentDetailById(id);
  ...
  const updatedRecord = await csrContentDetailService.updateCsrContentDetail(
    id,
    updatePayload,
  );
  ...
  successResponse(res, 200, "CSR Content updated successfully", updatedRecord);
});

export const remove = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await csrContentDetailService.getCsrContentDetailById(id);

  if (!item) {
    throw new ApiError(404, "CSR Content record not found");
  }
  ...
  await csrContentDetailService.deleteCsrContentDetail(id);
  successResponse(res, 200, "CSR Content record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject =
      await csrContentDetailService.updateCsrContentDetailSeq(id, payload);
    successResponse(res, 200, "CSR Content seq successfully", updatedProject);
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (csrContentDetailService as any).getById
      ? await (csrContentDetailService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (csrContentDetailService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
