...
import * as investorDocumentService from "./investorDocument.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  successResponse(res, 200, "InvestorDocuments created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  successResponse(
    res,
    200,
    "InvestorDocuments records fetched successfully",
    records,
  );
});
...
export const getOne = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await investorDocumentService.getInvestorDocumentsById(id);
  ...
  successResponse(res, 200, "Get edit InvestorDocuments record", record);
});

export const update = asyncHandler(async (req: Request, res: Response) => {
  const user = req.user as { id?: string };
  const id = req.params.id as string;
  const oldRecord = await investorDocumentService.getInvestorDocumentsById(id);
  ...
  const updatedRecord = await investorDocumentService.updateInvestorDocuments(
    id,
    updatePayload,
  );
  ...
  successResponse(
    res,
    200,
    "InvestorDocuments updated successfully",
    updatedRecord,
  );
});
...
export const remove = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await investorDocumentService.getInvestorDocumentsById(id);

  if (!item) {
    throw new ApiError(404, "InvestorDocuments record not found");
  }
  ...
  await investorDocumentService.deleteInvestorDocuments(id);
  successResponse(res, 200, "InvestorDocuments record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject =
      await investorDocumentService.updateInvestorDocumentsSeq(id, payload);
    successResponse(
      res,
      200,
      "InvestorDocuments seq successfully",
      updatedProject,
    );
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (investorDocumentService as any).getById
      ? await (investorDocumentService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (investorDocumentService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
