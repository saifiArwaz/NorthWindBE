...
import * as galleriesListServices from "./galleriesList.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await galleriesListServices.createGalleriesList({
    ...req.body,
  ...
  successResponse(res, 200, "Gallery created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await galleriesListServices.getAllList(
    page,
    limit,
    search,
    type,
  ...
  successResponse(res, 200, "Gallery records fetch successfully", records);
});

export const getOne = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const record = await galleriesListServices.getGalleriesListById(id);
  ...
  successResponse(res, 200, "Get edit Gallery record", record);
});
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await galleriesListServices.getGalleriesListById(id);
  ...
  const updatedRecord = await galleriesListServices.updateGalleriesList(
    id,
    updatePayload,
  );
...
export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await galleriesListServices.getGalleriesListById(id);
  ...
  await galleriesListServices.deleteGalleriesListById(id);
  successResponse(res, 200, "Gallery record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await galleriesListServices.updateProjectSeq(
      id,
      payload,
    );
    successResponse(res, 200, "Gallery seq successfully", updatedProject);
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (galleriesListServices as any).getById
      ? await (galleriesListServices as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (galleriesListServices as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
