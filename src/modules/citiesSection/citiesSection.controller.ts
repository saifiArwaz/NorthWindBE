...
import * as citiesSectionService from "./citiesSection.service.js";
import { successResponse } from "../../utils/responseHandler.utils.js";
import { ApiError } from "../../utils/apiError.utils.js";
import { deleteFromS3, getFileUrl } from "../../utils/fileHandling.utils.js";

export const createOrUpdateCitiesSection = asyncHandler(
  async (req: Request, res: Response) => {
    const user = req.user as { id?: string };

    const { cityId, sectionType } = req.body;
    const existedRecord =
      await citiesSectionService.getCitiesSectionBySectionType(
        cityId,
        sectionType,
      );
    ...
    if (existedRecord) {
      record = await citiesSectionService.updateCitiesSection(
        existedRecord.id,
        payload,
      );
    } else {
      record = await citiesSectionService.createCitiesSection(payload);
    }
...
export const getAll = asyncHandler(
  async (
    ...
    const record = await citiesSectionService.getAllCitiesSection(
      projectId,
      page,
      limit,
      search,
    ...
    successResponse(res, 200, "City Section fetched successfully", record);
  },
);

export const getCitiesSectionList = asyncHandler(
  async (req: Request, res: Response) => {
    const record = await citiesSectionService.getAllCitiesSectionList();
    successResponse(
      res,
      200,
      "All cities sections fetched successfully",
      record,
    );
  },
);
...
export const getOne = asyncHandler(
  async (
    req: Request<{ cityId: string; sectionType: string }>,
    res: Response,
  ) => {
    const { cityId, sectionType } = req.params;

    const record = await citiesSectionService.getCitiesSectionBySectionType(
      cityId,
      sectionType,
    );
    if (!record) {
      new ApiError(404, "City Section not found");
    }
...
export const remove = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await citiesSectionService.getCitiesSectionById(id as string);
  ...
  await citiesSectionService.deleteCitiesSection(id as string);

  successResponse(res, 200, "City Section deleted successfully");
});
...
export const destroySinglefile = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const pageSection = await citiesSectionService.getCitiesSectionById(id);
    if (!pageSection) {
    ...
    const updatedRecord = await citiesSectionService.updateCitiesSection(id, {
      files: filesObj,
    } as any);
...
export const changeSeq = asyncHandler(
  async (
    ...
    const record = await citiesSectionService.getCitiesSectionById(id);
    if (!record) {
      throw new ApiError(404, "Banner record not found");
    }

    const updatedProject = await citiesSectionService.updateSeq(id, payload);
    successResponse(res, 200, "City Section seq successfully", updatedProject);
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (citiesSectionService as any).getById
      ? await (citiesSectionService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (citiesSectionService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
