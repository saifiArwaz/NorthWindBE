...
import * as CareerGalleryServices from "./careerGallery.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await CareerGalleryServices.createCareerGallery({
    ...req.body,
  ...
  successResponse(res, 200, "Career Gallery created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await CareerGalleryServices.getAllList(page, limit, search);
  await Promise.all(
  ...
  successResponse(
    res,
    200,
    "Career Gallery records fetch successfully",
    records,
  );
});
...
export const getOne = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await CareerGalleryServices.getCareerGalleryById(id);
  ...
  successResponse(res, 200, "Get edit Career Gallery record", record);
});
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await CareerGalleryServices.getCareerGalleryById(id);
  ...
  const updatedRecord = await CareerGalleryServices.updateCareerGallery(
    id,
    updatePayload,
  );
  ...
  successResponse(
    res,
    200,
    "Career Gallery updated successfully",
    updatedRecord,
  );
});
...
export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await CareerGalleryServices.getCareerGalleryById(id);

  if (!item) {
    throw new ApiError(404, "Career Gallery record not found");
  }
  ...
  await CareerGalleryServices.deleteCareerGalleryById(id);
  successResponse(res, 200, "Career Gallery record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await CareerGalleryServices.updateProjectSeq(
      id,
      payload,
    );
    successResponse(
      res,
      200,
      "Career Gallery seq successfully",
      updatedProject,
    );
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (CareerGalleryServices as any).getById
      ? await (CareerGalleryServices as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (CareerGalleryServices as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
