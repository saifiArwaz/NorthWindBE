...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await teamService.createTeamSection({
    ...req.body,
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await teamService.getAllList(page, limit, search);
  await Promise.all(
...
export const getOne = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await teamService.getTeamSectionById(id);
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await teamService.getTeamSectionById(id);
  ...
  const updatedRecord = await teamService.updateTeamSection(id, updatePayload);
...
export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await teamService.getTeamSectionById(id);
  ...
  await teamService.deleteTeamSectionById(id);
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await teamService.updateProjectSeq(id, payload);
    successResponse(res, 200, "Project seq successfully", updatedProject);
  },
);

export const changeTeamFounder = asyncHandler(
  async (
    req: Request<{ id: string }, any, any, { type?: string }>,
    res: Response,
  ) => {
    ...
    if (isFounder !== "true" && isFounder !== "false") {
      throw new ApiError(400, "isFounder value must be a boolean");
    }
    payload.isFounder = Boolean(isFounder);
    ...
    const updatedTeam = await teamService.updateTeamFounder(id, payload);
    successResponse(res, 200, "Team founder successfully", updatedTeam);
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (teamService as any).getById
      ? await (teamService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (teamService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
