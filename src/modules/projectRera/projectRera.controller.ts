...
import * as projectReraService from "./projectRera.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await projectReraService.createProjectRera(data);
  if (record) {
  ...
  successResponse(res, 200, "Project Rera created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await projectReraService.getAllList(
    page,
    limit,
    search,
    projectId as string,
  ...
  successResponse(res, 200, "Project Rera records fetch successfully", records);
});

export const getOne = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const record = await projectReraService.getProjectReraById(id);
  ...
  successResponse(res, 200, "Get edit project Rera record", record);
});
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await projectReraService.getProjectReraById(id);
  ...
  /** 6. Attach updated files and audit fields */
  ...
  const updatedRecord = await projectReraService.updateProjectRera(
    id,
    updatePayload,
  );

  successResponse(res, 200, "Project Rera updated successfully", updatedRecord);
});

export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await projectReraService.getProjectReraById(id);

  if (!item) {
    throw new ApiError(404, "Project Rera record not found");
  }
  const fileFields = [];
  ...
  await projectReraService.deleteProjectRera(id);
  successResponse(res, 200, "Project Rera record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await projectReraService.updateSeq(id, payload);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (projectReraService as any).getById
      ? await (projectReraService as any).getById(id)
      : null;

    const updatedRecord = await (projectReraService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
