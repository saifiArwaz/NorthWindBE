...
import * as pageService from "./pageSections.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await pageService.createPageSection({
    ...req.body,
  ...
  successResponse(res, 200, "Page created successfully", record);
});

export const getPageSectionListByPageSlug = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const records = await pageService.getAllList(slug, page, limit, search);
    await Promise.all(
...
export const getPageById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await pageService.getPageSectionById(id);
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await pageService.getPageSectionById(id);
  if (!oldRecord) {
  ...
  /** 8. Update DB */
  const updatedRecord = await pageService.updatePageSection(id, updatePayload);
...
export const deleteById = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const page = await pageService.getPageSectionById(id);
  ...
  await pageService.deletePageSectionById(id);
  successResponse(res, 200, "Page Section record deleted successfully");
});
...
export const destroySinglefile = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const pageSection = await pageService.getPageSectionById(id);
    if (!pageSection) {
    ...
    const updatedRecord = await pageService.updatePageSection(id, {
      files: filesObj,
    });
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await pageService.updateSeq(
      id,
      payload.seq,
      payload.updatedBy,
    );
