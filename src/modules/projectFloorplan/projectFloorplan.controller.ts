...
import * as projectFloorplanService from "./projectFloorplan.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await projectFloorplanService.createProjectFloorplan(data);
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await projectFloorplanService.getAllList(
    page,
    limit,
    search,
    projectId as string,
...
export const getOne = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await projectFloorplanService.getProjectFloorplanById(id);
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  const oldRecord = await projectFloorplanService.getProjectFloorplanById(id);
  ...
  const updatedRecord = await projectFloorplanService.updateProjectFloorplan(
    id,
    updatePayload,
  );
...
export const destroy = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await projectFloorplanService.getProjectFloorplanById(id);
  ...
  await projectFloorplanService.deleteProjectFloorplan(id);
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await projectFloorplanService.updateSeq(id, payload);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (projectFloorplanService as any).getById
      ? await (projectFloorplanService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (projectFloorplanService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
