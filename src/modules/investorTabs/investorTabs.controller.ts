...
import * as investorTabService from "./investorTabs.service.js";
...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await investorTabService.createInvestorTabs({
    ...req.body,
  ...
  successResponse(res, 200, "InvestorTabs created successfully", record);
});
...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const records = await investorTabService.getAllList(page, limit, search);
  await Promise.all(
  ...
  successResponse(
    res,
    200,
    "InvestorTabs records fetched successfully",
    records,
  );
});
...
export const getOne = asyncHandler(async (req: Request, res: Response) => {
  ...
  const record = await investorTabService.getInvestorTabsById(id);
  ...
  successResponse(res, 200, "Get edit InvestorTabs record", record);
});

export const update = asyncHandler(async (req: Request, res: Response) => {
  const user = req.user as { id?: string };
  const id = req.params.id as string;
  const oldRecord = await investorTabService.getInvestorTabsById(id);
  ...
  const updatedRecord = await investorTabService.updateInvestorTabs(
    id,
    updatePayload,
  );
  ...
  successResponse(res, 200, "InvestorTabs updated successfully", updatedRecord);
});

export const remove = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const item = await investorTabService.getInvestorTabsById(id);

  if (!item) {
    throw new ApiError(404, "InvestorTabs record not found");
  }
  ...
  await investorTabService.deleteInvestorTabs(id);
  successResponse(res, 200, "InvestorTabs record deleted successfully");
});
...
export const changeSeq = asyncHandler(
  async (
    ...
    const updatedProject = await investorTabService.updateInvestorTabsSeq(
      id,
      payload,
    );
    successResponse(res, 200, "InvestorTabs seq successfully", updatedProject);
  },
);
...
export const changeStatus = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const record = (investorTabService as any).getById
      ? await (investorTabService as any).getById(id)
      : null;
    // We bypass getById check here if not universally named, the service updateStatus will throw if not found.

    const updatedRecord = await (investorTabService as any).updateStatus(
      id,
      status as boolean,
      user?.id,
    );
